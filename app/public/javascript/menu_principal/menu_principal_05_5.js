...
//IX1 = 2 * ESPES * (0.358 * RN ^ 3 + A2 * (A2 / 2 + RN) ^ 2 + 0.0833 * A2 ^ 3 + U * (A2 + 1.637 * RN) ^ 2 + 0.149 * RN ^ 3 + B2 * (A2 + 2 * RN) ^ 2) - 2 * AREA1 * YB ^ 2
...
//IX = IX + IX2
id('ixx').value = (IX1*1 + IX2*1).toFixed(2);
var IX = id('ixx').value;
...
id('iyy').value = (ESPES * Math.pow(B2,3) / 3 + 4 * c * RN * ESPES + A2 * Math.pow(ESPES,3) / 6 + FITA1 * Math.pow(ESPES,3) / 12 + 4 * ESPES * B2 * Math.pow(ESPES / 2 + ALTDB + B2 / 2,2) + 4 * U * ESPES * Math.pow(ESPES + RN - e * RN,2) + 2 * A2 * Math.pow(ESPES,3)).toFixed(2);
var IY = id('iyy').value;
...
id('wt').value =  ((2 * FITA + FITA1) * Math.pow(ESPES,2) / 3).toFixed(2);
var WT = id('wt').value;

//AREA1 = 2 * AREA1 + FITA1 * ESPES
id('area_total').value = (2 * AREA + FITA1 * ESPES).toFixed(2) || 0.00;

...
//IX = 4 * ESPES * (0.0417 * A2 ^ 3 + B2 * (A2 / 2 + RN) ^ 2 + 2 * U * (A2 / 2 + 0.637 * RN) ^ 2 + 2 * 0.149 * RN ^ 3 + 0.0833 * C2 ^ 3 + C2 / 4 * (A2 - C2) ^ 2) + ESPES * FITA1 ^ 3 / 12
form12.ixx.value = (4 * ESPES * (h * Math.pow(A2,3) + B2 * Math.pow(A2 / 2 + RN,2) + 2 * U * Math.pow(A2 / 2 + e * RN,2) + 2 * b * Math.pow(RN,3) + p * Math.pow(C2,3) + C2 / 4 * Math.pow(A2 - C2,2)) + ESPES * Math.pow(FITA1,3) / 12).toFixed(2);
var IX = form12.ixx.value;

//IY = C2 * ESPES ^ 3 / 3 + 8 * 0.149 * RN * ESPES + ESPES * B2 ^ 3 / 3 + A2 * ESPES ^ 3 / 6 + FITA1 * ESPES ^ 3 / 12 + 4 * C2 * ESPES * (BB + ESPES) ^ 2 + 4 * U * ESPES * (BB + ESPES - RN + 0.637 * RN) ^ 2 + 4 * U * ESPES * (RN + ESPES - 0.637 * RN) ^ 2 + 4 * B2 * ESPES * (B1 / 2 + ESPES / 2) ^ 2 + 2 * A2 * ESPES ^ 3
form13.iyy.value = (C2 * Math.pow(ESPES,3) / 3 + 8 * b * RN * ESPES + ESPES * Math.pow(B2,3) / 3 + A2 * Math.pow(ESPES,3) / 6 + FITA1 * Math.pow(ESPES,3) / 12 + 4 * C2 * ESPES * Math.pow(BB + ESPES,2) + 4 * U * ESPES * Math.pow(BB + ESPES - RN + 0.637 * RN,2) + 4 * U * ESPES * Math.pow(RN + ESPES - e * RN,2) + 4 * B2 * ESPES * Math.pow(B1 / 2 + ESPES / 2,2) + 2 * A2 * Math.pow(ESPES,3)).toFixed(2);
...
//CGX = B1 + ESPES / 2
form14.cgx.value = (B1 + ESPES / 2).toFixed(2); 
var CGX = form14.cgx.value;

//WY = IY / (B1 + ESPES / 2)
form13.wy.value = (IY / (B1 + ESPES / 2)).toFixed(2);
var WY = form13.wy.value;

//WT = (2 * FITA * ESPES ^ 2 + FITA1 * ESPES ^ 2) / 3
form14.wt.value =  ((2 * FITA * Math.pow(ESPES,2) + FITA1 * Math.pow(ESPES,2)) / 3).toFixed(2);
var WT = form14.wt.value;

//J1 = WT * ESPES
form12.j.value = (WT * ESPES).toFixed(2);
...
//AREA1 = AREA1 + FITA1 * ESPES
form19.area_total.value = (AREA + FITA1 * ESPES).toFixed(2);
...
form16.cw.value = (2 / 3 * ((WN0 * WN0 + WN0 * WN1 + WN1 * WN1) * ESPES * CB + (WN1 * WN1 + WN1 * WN2 + WN2 *  WN2) * ESPES * BB + (WN2 * WN2 + WN2 * WN3 + WN3 * WN3) * ESPES * AB + (WN3 * WN3 + WN3 * WN4 + WN4 * WN4) * ESPES * BB + (WN4 * WN4 + WN4 * WN5 + WN5 * WN5) * ESPES * CB)).toFixed(2);

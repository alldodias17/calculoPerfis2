...
//FITA2 = 2 * B4 + a4 + 2 * U
var fita02_1 = 2 * B4;
var fita02_2 = 2 * id('ud').value;
FITA2 = fita02_1 + a4 + fita02_2;
//AREA1 = FITA * ESPES
AREA01 = id('largura_da').value * id('espessura_d').value;
var AREA1 = id('area_total').value;
//AREA2 = FITA2 * ESPES
var AREAU = id('area_util').value;
AREA02 = FITA2 * id('espessura_d').value;
//XB = 2 * ESPES / AREA1 * (B2 * (B2 / 2 + RN) + U * 0.363 * RN)
XB = 2 * parseFloat(id('espessura_d').value) / parseFloat(AREA01) * (parseFloat(id('dtam_b').value) * (parseFloat(id('dtam_b').value) / 2 + parseFloat(id('raio_neutro').value)) + parseFloat(id('ud').value) * parseFloat(l) * parseFloat(id('raio_neutro').value));
//XB1 = 2 * ESPES / AREA2 * (B4 * (B4 / 2 + RN) + U * 0.363 * RN)
XB1 = 2 * parseFloat(id('espessura_d').value) / AREA02 * (B4 * (B4 / 2 + parseFloat(id('raio_neutro').value)) + parseFloat(id('ud').value) * parseFloat(l) * parseFloat(id('raio_neutro').value));
//CGX1 = XB + ESPES / 2
CGX1 = (XB*1 + id('espessura_d').value / 2).toFixed(2);
//CGX2 = XB1 + ESPES / 2
CGX2 = (XB1*1 + id('espessura_d').value / 2).toFixed(2);
//CGX = (CGX1 * AREA1 + AREA2 * (B1 - CGX2)) / (AREA1 + AREA2)
var cgx1 = id('dtam_B').value - CGX2;
var cgx2 = AREA01 + AREA02;
var cgx3 = CGX1 * AREA01;
var cgx4 = AREA02 * cgx1;
var cgx5 = cgx3 + cgx4;
id('cgx').value = (cgx5 / cgx2).toFixed(2);
...
//XG1 = CGX - CGX1
XG1 = id('cgx').value - CGX1;
//XG2 = (B1 - CGX) - CGX2
XG2 = (id('dtam_B').value - id('cgx').value) - CGX2;
//Cw01 = (ESPES * AB ^ 2 * BB ^ 3) / 12 * ((3 * BB + 2 * AB) / (6 * BB + AB))
Cw01 = (parseFloat(ESPES) * Math.pow(id('dtam_a1').value,2) * Math.pow(id('dtam_bbarra').value,3)) / 12 * ((3 * parseFloat(id('dtam_bbarra').value) + 2 * parseFloat(id('dtam_a1').value)) / (6 * parseFloat(id('dtam_bbarra').value) + parseFloat(id('dtam_a1').value)));
...
//Cw = Cw - CW1
id('cw').value = (Cw01 - Cw02).toFixed(2);
//IX = IX + IX2
id('ixx').value = IX1*1 + IX2*1;
var IX = parseFloat(id('ixx').value);
//WX = IX / CGY
id('wx').value = id('ixx').value / id('cgy').value;
//IY = IY + IY2
id('iyy').value = (IY1*1 + IY2*1).toFixed(2);
var IY = parseFloat(id('iyy').value);
//WY = IY / CGX
id('wy').value = (id('iyy').value / id('cgx').value).toFixed(2);
//WT = 2 * (A1 - ESPES) * (B1 - ESPES) * ESPES
id('wt').value =  2 * (parseFloat(A1) - parseFloat(ESPES)) * (parseFloat(B1) - parseFloat(ESPES)) * parseFloat(ESPES);
//AREA1 = AREA1 + AREA2
id('area_total').value = AREA01 + AREA02;
//AREAU = AREA1
...
id('ip').value =  IX + IY + Math.pow(DIST,2) * AREA01;
var IP = id('ip').value;
//KGM = AREA1 * 0.001 * 7.85
id('d_Peso').value = (id('area_total').value * s * t).toFixed(2);
var KGM = id('d_Peso').value;
//IX1 = (IX / AREA1) ^ 0.5
id('ixis').value = Math.pow(IX / id('area_total').value,0.5).toFixed(2);
//IY1 = (IY / AREA1) ^ 0.5
id('iy').value = Math.pow(IY / id('area_total').value,0.5).toFixed(2);
